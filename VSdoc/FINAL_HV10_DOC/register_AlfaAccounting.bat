...
"%~dp0HelpLibManagerLauncher.exe" /product "VS" /version "100" /uninstall /silent /vendor "VendorName" /mediaBookList "AlfaAccounting Reference" /productName "AlfaAccounting Reference" > NUL
REM Register 
...
REM First unregister if it was registered before.
"%MSHV20_ROOT_PATH%HlpCtntmgr.exe" /operation uninstall /catalogName VisualStudio11 /locale %locale% /vendor "VendorName"  /productName "AlfaAccounting Reference" /bookList "AlfaAccounting Reference" /wait 0
...
REM First unregister if it was registered before.
"%MSHV21_ROOT_PATH%HlpCtntmgr.exe" /operation uninstall /catalogName VisualStudio12 /locale %locale% /vendor "VendorName"  /productName "AlfaAccounting Reference" /bookList "AlfaAccounting Reference" /wait 0
echo HlpCtntmgr.exe uninstall exit code=%ERRORLEVEL%
REM Register 
REM If calling from outside of a batch script (make sure you specify a correct full path to your .msha file.), e.g:
...
REM First unregister if it was registered before.
"%MSHV22_ROOT_PATH%HlpCtntmgr.exe" /operation uninstall /catalogName VisualStudio14 /locale %locale% /vendor "VendorName"  /productName "AlfaAccounting Reference" /bookList "AlfaAccounting Reference" /wait 0
echo HlpCtntmgr.exe uninstall exit code=%ERRORLEVEL%
REM Register 
REM If calling from outside of a batch script (make sure you specify a correct full path to your .msha file.), e.g:
...
:MSHV23
REM ************* VS 2017
REM Determine whether MS Help Viewer 2.3 is installed
set KEY_NAME=%ROOT_KEY_NAME%\v2.3
set VALUE_NAME=AppRoot
for /F "usebackq tokens=2*" %%A IN (`reg query "%KEY_NAME%" /v "%VALUE_NAME%" 2^>nul ^| find "%VALUE_NAME%"`) do (
  set MSHV23_ROOT_PATH=%%B
)
if not defined MSHV23_ROOT_PATH (
	goto MSHV_CONTINUE
)
REM MS Help Viewer 2.3 is present, register the doc under it

REM Get installed locale of VisualStudio15 catalog
set KEY_NAME=%KEY_NAME%\Catalogs\VisualStudio15
REM Set SUB_KEYS to all lines with sub-keys (that have a locale at the end)
for /f "tokens=*" %%a in ('reg query "%KEY_NAME%" ^| find "%KEY_NAME%\"') do (set SUB_KEYS=%%a)
REM Extract the locale after the last \ character.
for %%a in (%SUB_KEYS%) do set locale=%%~nxa
...
REM First unregister if it was registered before.
"%MSHV23_ROOT_PATH%HlpCtntmgr.exe" /operation uninstall /catalogName VisualStudio15 /locale %locale% /vendor "VendorName"  /productName "AlfaAccounting Reference" /bookList "AlfaAccounting Reference" /wait 0
echo HlpCtntmgr.exe uninstall exit code=%ERRORLEVEL%
REM Register 
REM If calling from outside of a batch script (make sure you specify a correct full path to your .msha file.), e.g:
REM "HlpCtntmgr.exe" /operation install /catalogName VisualStudio15 /locale en-US /sourceUri "C:\FINAL_HV10_DOC\helpcontentsetup.msha" /wait 0
REM If calling from a batch script (%~dp0 expands to BAT's drive and path):
"%MSHV23_ROOT_PATH%HlpCtntmgr.exe" /operation install /catalogName VisualStudio15 /locale %locale% /sourceUri "%~dp0helpcontentsetup.msha" /wait 0
echo HlpCtntmgr.exe install exit code=%ERRORLEVEL%
